...
public class LatLonPointDistanceQuery  extends Query {
	  ...
	  public Weight createWeight(IndexSearcher searcher, ScoreMode scoreMode, float boost) throws IOException {
	    ...
	    return (Weight)new ConstantScoreWeight(this,boost) {
	        public Scorer scorer(LeafReaderContext context) throws IOException {
	          LeafReader reader = context.reader();
	          PointValues values = reader.getPointValues(LatLonPointDistanceQuery.this.field);
	          if (values == null)
	            return null; 
	          FieldInfo fieldInfo = reader.getFieldInfos().fieldInfo(LatLonPointDistanceQuery.this.field);
	          if (fieldInfo == null)
	            return null; 
	          LatLonPointField.checkCompatible(fieldInfo);
	          ...
	          values.intersect(new PointValues.IntersectVisitor() {
	                ...
	                public void visit(int docID, byte[] packedValue) {
	                  ...
	                  if (SloppyMath.haversinSortKey(LatLonPointDistanceQuery.this.latitude, LatLonPointDistanceQuery.this.longitude, docLatitude, docLongitude) <= sortKey)
	                    this.adder.add(docID); 
	                }
	                ...
	                public PointValues.Relation compare(byte[] minPackedValue, byte[] maxPackedValue) {
	                  ...
	                  if ((LatLonPointDistanceQuery.this.longitude < lonMin || LatLonPointDistanceQuery.this.longitude > lonMax) && (axisLat + 8.993203677616636E-7D < latMin || axisLat - 8.993203677616636E-7D > latMax))
	                    if (SloppyMath.haversinSortKey(LatLonPointDistanceQuery.this.latitude, LatLonPointDistanceQuery.this.longitude, latMin, lonMin) > sortKey && 
	                      SloppyMath.haversinSortKey(LatLonPointDistanceQuery.this.latitude, LatLonPointDistanceQuery.this.longitude, latMin, lonMax) > sortKey && 
	                      SloppyMath.haversinSortKey(LatLonPointDistanceQuery.this.latitude, LatLonPointDistanceQuery.this.longitude, latMax, lonMin) > sortKey && 
	                      SloppyMath.haversinSortKey(LatLonPointDistanceQuery.this.latitude, LatLonPointDistanceQuery.this.longitude, latMax, lonMax) > sortKey)
	                      return PointValues.Relation.CELL_OUTSIDE_QUERY;  
	                  if (lonMax - LatLonPointDistanceQuery.this.longitude < 90.0D && LatLonPointDistanceQuery.this.longitude - lonMin < 90.0D && 
	                    SloppyMath.haversinSortKey(LatLonPointDistanceQuery.this.latitude, LatLonPointDistanceQuery.this.longitude, latMin, lonMin) <= sortKey && 
	                    SloppyMath.haversinSortKey(LatLonPointDistanceQuery.this.latitude, LatLonPointDistanceQuery.this.longitude, latMin, lonMax) <= sortKey && 
	                    SloppyMath.haversinSortKey(LatLonPointDistanceQuery.this.latitude, LatLonPointDistanceQuery.this.longitude, latMax, lonMin) <= sortKey && 
	                    SloppyMath.haversinSortKey(LatLonPointDistanceQuery.this.latitude, LatLonPointDistanceQuery.this.longitude, latMax, lonMax) <= sortKey)
	  ...
	  public boolean equals(Object other) {
	    return (sameClassAs(other) && 
	      equalsTo((LatLonPointDistanceQuery)getClass().cast(other)));
	  }
